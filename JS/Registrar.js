...
async function emailValidation(nombre, correo, password) {
  const valuEmail = document.getElementById("useremail").value;
  const date = await fetch("http://129.151.108.133:8080/api/user/" + valuEmail);
  const date_reponseJsonFormat = await date.json();
  console.log(date_reponseJsonFormat);

  const pass1 = document.getElementById("password").value;
  const pass2 = document.getElementById("passwordrepeat").value;

  if (!date_reponseJsonFormat) {
    if (expresiones.correo.test(valuEmail)) {
      if (pass1 == pass2) {
        if (nombre && correo && password) {
          ...
          $.ajax({
            ...
            url: "http://129.151.108.133:8080/api/user/new",
            data: datosPeticion,
            type: "POST",
